...
const ROLES: { value: 'merchant' | 'admin'; title: string; hint: string }[] = [
  { value: 'merchant', title: 'Shop user', hint: 'Creates orders and tracks parcels' },
  { value: 'admin', title: 'Admin', hint: 'Sees every order and manages users' },
];
...
function AddUserForm({ onAdded, onCancel }: { onAdded: (user: UserOut) => void; onCancel: () => void }) {
  ...
  const input = 'el-input h-[52px] text-[17px]';
  ...
  return (
    <form onSubmit={submit} className="rounded-xl border-2 border-brand bg-white p-6" noValidate>
