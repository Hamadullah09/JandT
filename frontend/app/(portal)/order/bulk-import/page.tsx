...
function Result({ summary }: { summary: BulkCommitOut }) {
  ...
  return (
    <section
      role="status"
      className={`rounded-xl border-2 p-6 ${created ? 'border-[#67c23a] bg-[#f0f9eb]' : 'border-[#fbc4c4] bg-danger-tint'}`}
    >
      <div className="flex items-center gap-4">
        {created && <CheckCircleIcon className="h-14 w-14 shrink-0 text-[#3f8f1f]" />}
        <div>
          <h2 className={`text-[32px] font-bold leading-tight ${created ? 'text-[#2f6f14]' : 'text-danger'}`}>
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            {packing.map((file) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex min-h-[64px] items-center gap-4 rounded-lg bg-brand px-5 py-3 text-white transition-colors hover:bg-[#2e2d2a]"
              >
                <DownloadIcon className="h-7 w-7 shrink-0" />
                <span>
                  <span className="block text-[18px] font-bold">{file.title}</span>
...
export default function BulkImportPage() {
  ...
  return (
    <div className="mx-auto max-w-[1500px] space-y-6 p-6">
      ...
      {/* ------------------------------------------------------- step 1 */}
      <section className="rounded-xl border-2 border-line bg-white p-6">
        <Step number={1} title="Choose your CSV file">
          <div
            ...
            onDrop={onDrop}
            className={`mt-4 flex cursor-pointer flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed px-6 py-10 text-center transition-colors ${
        ...
        <Step number={3} title="Create the orders">
          {running ? (
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              <button
                type="button"
                className="el-btn el-btn-primary h-[60px] px-8 text-[20px] font-bold"
              ...
              {selection.size > 0 && (
                <>
                  <button type="button" className="el-btn h-[60px] px-6 text-[18px]" onClick={() => void create(true)}>
                    Create only the {selection.size} ticked
                  </button>
                  <button type="button" className="el-btn h-[60px] px-6 text-[18px]" onClick={() => void removeTicked()}>
