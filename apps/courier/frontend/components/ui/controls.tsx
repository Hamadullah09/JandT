...
export function Field({
  ...
  label: string;
  required?: boolean;
  /** Columns occupied in the 5-column form grid; a phone stacks every field. */
  span?: 1 | 2 | 3;
  suffix?: ReactNode;
  children: ReactNode;
  hint?: string;
}) {
  // the grid is one column below `md`, so the spans only start there
...
export function PhoneInput({
  ...
  return (
    <div className="flex">
      ...
      <input
        {...rest}
        className={`el-input min-w-0 rounded-l-none ${invalid ? 'border-danger' : ''}`}
      />
    </div>
  );
}
...
export function HeaderAction({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex min-h-[40px] items-center gap-1.5 text-base text-text-regular hover:text-brand md:min-h-0"
    >
      {icon}
      {label}
    </button>
