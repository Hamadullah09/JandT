...
export default function TrackingPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-[1246px] px-4 pb-4 md:px-0">
      <h1 className="mt-10 break-words text-center text-[26px] font-bold uppercase leading-[34px] tracking-[1.5px] text-brand sm:text-[30px] sm:leading-[40px]">
        Track your order
      </h1>

      <form
        className="mx-auto mt-6 flex h-[68px] w-full max-w-[860px] overflow-hidden border border-brand bg-white"
        onSubmit={(event) => {
      ...
      >
        <input
          ...
          className="min-w-0 flex-1 bg-transparent pl-4 text-[18px] text-brand outline-none placeholder:text-[#8a8984] sm:pl-6"
          autoComplete="off"
          spellCheck={false}
        />
        <button
          type="submit"
          className="flex w-[96px] shrink-0 items-center justify-center bg-brand text-[16px] font-bold uppercase tracking-[1.5px] text-white hover:bg-[#2e2d2a] sm:w-[120px]"
          disabled={loading}
        >
