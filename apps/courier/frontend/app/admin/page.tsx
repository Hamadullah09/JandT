  ...
  return (
    ...
    >
      <span className="flex items-center gap-3">
        ...
        <span className="break-words text-[30px] font-bold leading-none text-text-primary sm:text-[34px]">{count}</span>
      </span>
...
export default function AdminDashboardPage() {
  ...
  // Arriving from the warehouse's "Open in courier admin" (?q=<tracking no>):
  // search for the parcel and open it.
  useEffect(() => {
    const q = new URLSearchParams(window.location.search).get('q')?.trim();
    if (!q) return;
    setSearch(q);
    if (/^\d{12}$/.test(q)) setOpenOrder(q);
  }, []);
  ...
  return (
    <div className="space-y-6 px-4 py-5 sm:px-6 sm:py-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex max-w-full flex-wrap overflow-hidden rounded-lg border-2 border-line bg-white sm:flex-nowrap">
          {PERIODS.map((period) => {
            ...
            return (
              <button
                ...
                className={`h-11 basis-1/2 border-r-2 border-line px-4 text-[16px] last:border-r-0 sm:basis-auto ${
      ...
      {toast && (
        <div
          role="status"
          className="fixed bottom-6 left-1/2 z-50 max-w-[calc(100vw-2rem)] -translate-x-1/2 rounded-lg bg-[#303133] px-6 py-4 text-[17px] text-white shadow-xl"
        >
          {toast}
        </div>
      )}
