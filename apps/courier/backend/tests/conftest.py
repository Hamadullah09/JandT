...
TEST_DATABASE_URL = os.environ.get(
    "JT_TEST_DATABASE_URL", "postgresql+asyncpg://inaaya:inaaya@localhost:5433/inaaya_test"
)
    ...
    async def probe() -> bool:
        ...
        try:
            return bool(await conn.fetchval(
                "select to_regclass($1) is not null and to_regclass('core.users') is not null",
                f"{settings.db_schema}.orders",
            ))
        finally:
            await conn.close()
