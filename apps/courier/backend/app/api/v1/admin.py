...
async def warehouse_links(
    session: AsyncSession, tracking_nos: Iterable[str]
) -> dict[str, dict[str, Any]]:
    """Which warehouse order each parcel was booked from.

    Read from the warehouse module's tables, which this database role is
    granted read access to.  Inside a savepoint, so a database without the
    warehouse module (a test database, a courier-only install) simply has no
    links instead of failing the page.
    """
    wanted = [no for no in set(tracking_nos) if no]
    if not wanted:
        return {}
    try:
        async with session.begin_nested():
            rows = await session.execute(
                text(
                    "SELECT s.tracking_no, o.id, o.order_no, o.status "
                    "  FROM warehouse.shipments s JOIN warehouse.orders o ON o.id = s.order_id "
                    " WHERE s.tracking_no = ANY(:wanted)"
                ),
                {"wanted": wanted},
            )
            return {
                row.tracking_no: {
                    "warehouse_order_id": row.id,
                    "warehouse_order_no": row.order_no,
                    "warehouse_status": row.status,
                }
                for row in rows
            }
    except DBAPIError:
...
def admin_order(
    order: Order, last: TrackingEvent | None, link: dict[str, Any] | None = None
) -> AdminOrderOut:
    fields = {
        "items": order.items,
...
async def list_orders(
    ...
    links = await warehouse_links(session, (o.tracking_no for o in orders))
    ...
    return AdminOrderPage(
        items=[
            admin_order(order, tracking_events_latest(events[order.id]), links.get(order.tracking_no))
            for order in orders
        ],
        page=page,
        size=size,
...
async def order_detail(
    ...
    links = await warehouse_links(session, [order.tracking_no])
    return AdminOrderDetailOut(
        order=admin_order(order, tracking_events_latest(events), links.get(order.tracking_no)),
