...
class AdminOrderOut(BaseModel):
    ...
    tracking_url: str
    waybill_url: str
    #: the warehouse order this parcel was booked from, when it was
    warehouse_order_id: int | None = None
...
class MeOut(BaseModel):
    #: the platform account id - the same number on the warehouse
    id: int | None = None
    username: str
    name: str
    #: admin, operator or merchant
    role: str
...
class UserCreateIn(BaseModel):
    ...
    email: str = Field(default="", max_length=254)
    password: str = Field(default="", max_length=128)
    #: admin (everything), operator (warehouse staff + this portal), merchant (this portal)
    role: Literal["admin", "operator", "merchant"] = "merchant"
