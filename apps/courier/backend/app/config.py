...
class Settings(BaseSettings):
    ...
    #: development | production.  Production refuses to start without a real
    #: signing secret, and marks the login cookie Secure when told to.
    environment: str = "development"

    # One PostgreSQL database for the whole platform (see database/init): this
    # module's tables live in their own schema, next to `core` (the shared
    # accounts) and `warehouse`.
    database_url: str = "postgresql+asyncpg://courier_app:courier@localhost:5433/inaaya"
    db_schema: str = "courier"

    # ---- platform sign-in ------------------------------------------------
    # The secret every platform token is signed with. The warehouse API must be
    # given the same value (AUTH_JWT_SECRET in the platform .env): that is what
    # makes one login work on both modules.
    auth_jwt_secret: str = ""
    #: how long a login on the portal lasts
    auth_session_hours: int = 24 * 7
    #: send the login cookie over HTTPS only (every public deployment)
    ...
    def _schema_name(cls, v: str) -> str:
        if not v.replace("_", "").isalnum():
            raise ValueError("db_schema must be a plain identifier")
        return v
    ...
    def search_path(self) -> str:
        """This module first, then the shared accounts, then extensions (citext)."""
        return f"{self.db_schema},core,public"

    @property
    def signing_secret(self) -> str:
        """The platform token secret, or - in development only - a stand-in.

        A token signed with a key anyone can read is not a token, so production
        refuses to start without a real one.  Development gets a fixed local
        key so the portal works out of the box; sign-ins then only carry over to
        the warehouse if it is given the same JT_AUTH_JWT_SECRET.
        """
        ...
        if self.is_production:
            raise RuntimeError(
                "JT_AUTH_JWT_SECRET is not set (at least 32 characters). Use the platform's "
                "AUTH_JWT_SECRET so the warehouse and the courier portal accept the same logins."
            )
        return "development-only-signing-key-do-not-deploy-000000"
